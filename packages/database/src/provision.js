 ...
 */
async function provisionTenant(name, slug, region, isolatedDbUrl, tier = 'CLINIC', settings, enabledModules, admin) {
    console.log(`[Provision] Starting provisioning for tenant: ${name} (${slug})`);
    // Create a unique reference ID for KMS encryption
    const keyRef = `kms-amisi-${crypto_1.default.randomBytes(16).toString('hex')}`;
    // Generate a shared secret for Edge-to-Cloud synchronization
    const sharedSecret = crypto_1.default.randomBytes(32).toString('hex');
    const controlDb = (0, index_1.getControlDb)();
    ...
    try {
        ...
        (0, child_process_1.execSync)(`cmd.exe /c npx prisma db push --schema ./prisma/tenant.prisma`, {
            env: {
                ...process.env,
        ...
        const isolatedClient = new index_1.TenantClient({
...
function getDefaultModulesForTier(tier) {
    const base = {
        pmi: true,
        opd: true,
        pharmacy: true,
...
async function seedAdminUser(dbUrl, admin) {
    const isolatedClient = new index_1.TenantClient({
        datasourceUrl: dbUrl
    });
    await isolatedClient.$connect();
...
if (require.main === module) {
    ...
    provisionTenant(name, slug, region, dbUrl, tier || 'CLINIC')
        .then(() => process.exit(0))
        .catch(e => {
        console.error(e);
        process.exit(1);
