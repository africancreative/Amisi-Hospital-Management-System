...
// ─── WEB CONTACT & PROVISIONING ACTIONS ──────────────────────────────────────
...
export async function getContactMessages(): Promise<any[]> {
    ...
    return leads.map((l: any) => {
        ...
        try {
            if (l.customConfig) {
                custom = typeof l.customConfig === 'string' ? JSON.parse(l.customConfig) : l.customConfig;
            }
        } catch {}
        
        const isStarred = !!custom?.starred;
        const status = custom?.status || (l.status === 'Won' ? 'Provisioned' : l.status === 'NewLead' ? 'Unread' : l.status);
        const isOnboarding = l.tags?.includes('onboarding-request');
        ...
        return {
            ...
            subject: isOnboarding
                ? `Enterprise Onboarding Request: ${l.hospitalName}`
                : (custom?.subject || `Inquiry from ${l.contactName}`),
            message: l.message || 'No message details provided.',
            status: status,
            date: new Date(l.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
            rawDate: l.createdAt,
            starred: isStarred,
...
export async function submitContactForm(data: {
    ...
    const lead = await controlDb.lead.create({
        data: {
            hospitalName: data.organization || `${data.name}'s Organization`,
            contactName: data.name,
            contactEmail: data.email,
            contactPhone: data.phone || null,
            source: 'Website',
            status: 'NewLead',
            facilityType: (data.facilityType as any) || 'CLINIC',
...
export async function updateContactMessageStatus(id: string, updates: { status?: string; starred?: boolean }): Promise<any> {
    const controlDb = getControlDb();
    const lead = await controlDb.lead.findUnique({ where: { id } });
    if (!lead) throw new Error('Message not found');

    let custom: any = {};
    try {
        if (lead.customConfig) {
            custom = typeof lead.customConfig === 'string' ? JSON.parse(lead.customConfig) : lead.customConfig;
        }
    } catch {}

    if (updates.status !== undefined) custom.status = updates.status;
    if (updates.starred !== undefined) custom.starred = updates.starred;

    let leadStatus = lead.status;
    if (updates.status === 'Replied') leadStatus = 'ProposalSent';
    if (updates.status === 'Archived') leadStatus = 'Lost';

    await controlDb.lead.update({
        where: { id },
        data: {
            status: leadStatus,
            customConfig: JSON.stringify(custom)
        }
    });
...
export async function provisionTenantFromLead(leadId: string): Promise<any> {
    await ensureSuperAdmin();
    const controlDb = getControlDb();
    const lead = await controlDb.lead.findUnique({ where: { id: leadId } });
    if (!lead) throw new Error('Onboarding request lead not found');

    const rawName = lead.hospitalName || lead.contactName || 'Hospital Node';
    ...
    let slug = baseSlug;
    const existing = await controlDb.tenant.findUnique({ where: { slug } });
    if (existing) {
        slug = `${baseSlug}-${Math.floor(100 + Math.random() * 900)}`;
    }

    const name = lead.hospitalName || `${lead.contactName}'s Hospital`;
    const region = 'us-east-1';
    const tier = (lead.facilityType as any) || 'CLINIC';
    ...
    try {
        ...
        await provisionTenant(
            ...
            '', // isolatedDbUrl
            tier,
            { contactEmail: lead.contactEmail, phone: lead.contactPhone },
            {},
            { name: lead.contactName || 'Hospital Admin', email: lead.contactEmail, passwordHash: '$2b$10$dummyHashForDefaultAdminPassword' }
        );
    } catch (provErr) {
        console.warn('[provisionTenantFromLead] Automatic provisioner script warning, creating tenant fallback:', provErr);
        const existingTenant = await controlDb.tenant.findUnique({ where: { slug } });
        if (!existingTenant) {
            await controlDb.tenant.create({
                data: {
                    ...
                    slug,
                    dbUrl: '',
                    encryptionKeyReference: `kms-${slug}`,
                    region,
                    tier,
                    status: 'active',
                    enabledModules: {}
    ...
    // Update lead status to Won & mark provisioned
    let custom: any = {};
    try {
        if (lead.customConfig) {
            custom = typeof lead.customConfig === 'string' ? JSON.parse(lead.customConfig) : lead.customConfig;
    ...
    custom.status = 'Provisioned';
    custom.provisionedAt = new Date().toISOString();

    await controlDb.lead.update({
        where: { id: leadId },
        data: {
            status: 'Won',
            convertedTenantId: tenant?.id || null,
            customConfig: JSON.stringify(custom)
        }
    });
    ...
    return { success: true, tenantSlug: slug, tenantId: tenant?.id, tenantName: name };
}
