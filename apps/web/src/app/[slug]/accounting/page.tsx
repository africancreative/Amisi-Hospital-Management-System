...
export default async function AccountingDashboard({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const assetBalance = trialBalance.filter((a) => a.type === 'ASSET').reduce((acc: number, a) => acc + a.balance, 0);
    const revenueBalance = trialBalance.filter((a) => a.type === 'REVENUE').reduce((acc: number, a) => acc + a.balance, 0);
    const expenseBalance = trialBalance.filter((a) => a.type === 'EXPENSE').reduce((acc: number, a) => acc + a.balance, 0);
                ...
                <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        ...
                        <p className="text-gray-500 font-medium mt-2 uppercase text-xs tracking-[0.2em]">
                            IFRS-Compliant Double-Entry Accounting Core for <span className="text-violet-500 font-bold uppercase">{slug.replace('-', ' ')}</span>
                        </p>
                    </div>

                    <div className="flex gap-4">
                        <Link
                            href={`/${slug}/accounting/coa`}
                    ...
                    <div className="lg:col-span-2 space-y-8">
                        <section className="bg-white dark:bg-gray-950 rounded-[40px] border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                            <div className="px-8 py-6 border-b border-gray-100 dark:border-gray-900 flex justify-between items-center">
                                ...
                                <Link href={`/${slug}/accounting/ledger`} className="text-xs font-black text-violet-500 hover:underline">View All</Link>
                            </div>
                            <div className="p-0">
                                                ...
                                                <td className="px-8 py-6">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5" suppressHydrationWarning>{new Date(entry.date).toLocaleDateString()} &bull; REF: {entry.reference || 'N/A'}</p>
                                                        </div>
                                                    </div>
                                                </td>
                    ...
                    <div className="space-y-8">
                        <section className="bg-violet-950 text-white rounded-[40px] p-8 shadow-2xl shadow-violet-900/40 relative overflow-hidden">
                            ...
                            <div className="space-y-6">
                                <ReportLink label="Trial Balance" icon={FileText} href={`/${slug}/accounting/reports/trial-balance`} />
                                <ReportLink label="Profit & Loss" icon={TrendingUp} href={`/${slug}/accounting/reports/pl`} />
                                <ReportLink label="Balance Sheet" icon={BarChart3} href={`/${slug}/accounting/reports/balance-sheet`} />
                                <ReportLink label="Cash Flow" icon={PieChart} href={`/${slug}/accounting/reports/cash-flow`} />
                            </div>
                        </section>
...
function FinStatCard({ icon: Icon, label, value, color, trend }: FinStatCardProps) {
    const colors = {
...
function ReportLink({ label, icon: Icon, href }: { label: string, icon: any, href: string }) {
    return (
