...
export default async function LabEntryPage({ params }: { params: { id: string } }) {
    ...
    async function handleSubmit(formData: FormData) {
        ...
        await recordLabResult(params.id, results, technicianName);
        redirect('/lab');
    }
            ...
            <div className="mx-auto max-w-3xl">
                <header className="mb-10">
                    ...
                    <p className="text-gray-500 font-medium mt-1 uppercase text-xs tracking-widest">
                        Test: {order.testName} • Patient: {order.patient.lastName}, {order.patient.firstName}
                    </p>
                </header>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-blue-50/50 dark:bg-blue-900/10 rounded-2xl border border-blue-100 dark:border-blue-900/50">
                            <div className="md:col-span-2">
                                <label className="block">
                                    ...
                                    <input name="param1_name" defaultValue={order.testName} className="mt-2 block w-full bg-white dark:bg-gray-900 border-0 rounded-xl px-4 py-3 font-bold" />
                                </label>
                            </div>
                            <div>
                                <label className="block">
