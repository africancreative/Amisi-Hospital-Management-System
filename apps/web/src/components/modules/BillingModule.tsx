    ...
    <div className="flex flex-col h-full bg-gray-950 text-slate-100 font-sans">
      <header className="p-6 border-b border-white/5 flex justify-between items-center bg-gray-900/50">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-blue-600/20 rounded-lg"><Receipt className="text-blue-500" size={24} /></div>
          <div>
                        ...
                        <div className="bg-white/[0.02] border border-white/10 rounded-3xl overflow-hidden">
                           <table className="w-full text-left text-sm">
                              ...
                              <tbody className="divide-y divide-white/5">
                                 {activeInvoice.billItems.map((item: any) => (
                                    <tr key={item.id} className="hover:bg-white/[0.01]">
                                       {(!autoAllocate) && (
                                         <td className="p-4">
                                            {item.status !== 'PAID' && (
                                              <input type="number" className="w-20 bg-white/5 border border-white/20 rounded p-1 text-xs" value={allocations[item.id] || ''} onChange={(e) => handleAllocationChange(item.id, Number(e.target.value))} />
                                            )}
                                         </td>
                                       )}
                                       <td className="p-4">
                                          <p className="font-bold text-white flex items-center gap-2">{item.description}{item.isExempt && <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-500 text-[8px] uppercase tracking-wider">Exempt</span>}</p>
                                       ...
                                       <td className="p-4 text-center"><span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase ${item.status === 'PAID' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>{item.status}</span></td>
                                    </tr>
                                 ))}
                              </tbody>
                           </table>
                     ...
                     <div className="grid grid-cols-2 gap-8">
                        <section className="space-y-4">
                           ...
                           <div className="bg-indigo-500/5 border border-indigo-500/10 p-6 rounded-3xl space-y-4">
                               <div className="flex gap-4">
                                  <button onClick={() => setAutoAllocate(true)} className={`flex-1 py-3 rounded-2xl text-sm font-bold shadow-xl transition-all ${autoAllocate ? 'bg-indigo-600 text-white' : 'bg-white/5 border border-indigo-500/50 text-slate-300'}`}>Waterfall (Auto)</button>
                                  <button onClick={() => setAutoAllocate(false)} className={`flex-1 py-3 rounded-2xl text-sm font-bold shadow-xl transition-all ${!autoAllocate ? 'bg-indigo-600 text-white' : 'bg-white/5 border border-indigo-500/50 text-slate-300'}`}>Per-Item Selection</button>
                               </div>
                           </div>
                        </section>
                        <section className="space-y-4">
                           ...
                           <div className="bg-emerald-500/5 border border-emerald-500/10 p-6 rounded-3xl space-y-4">
                               <div className="relative flex items-center gap-2">
                                  <select className="bg-white/5 border border-white/20 rounded-xl py-4 px-3 text-sm font-black text-emerald-500" value={paymentCurrency} onChange={e => setPaymentCurrency(e.target.value)}><option value="USD">USD</option><option value="KSh">KSh</option></select>
                                  <input type="number" value={paymentAmount} disabled={!autoAllocate} onChange={(e) => setPaymentAmount(e.target.value)} className="w-full bg-white/5 border border-emerald-500/20 rounded-xl py-4 px-4 text-2xl font-black focus:outline-none disabled:opacity-50" />
                               </div>
                               <button onClick={handlePostPayment} disabled={recordPayment.isPending || !paymentAmount} className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 rounded-2xl font-black text-lg text-white shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 disabled:opacity-50"><CreditCard size={24} /> {recordPayment.isPending ? 'Processing...' : 'Post Payment'}</button>
                           </div>
                        </section>
                     </div>
                  </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
           <div className="bg-gray-900 border border-white/10 p-8 rounded-3xl w-full max-w-xl">
              <div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-black text-white">Add Billed Service</h2><button onClick={() => setShowAddModal(false)}><X size={24} className="text-slate-500 hover:text-white" /></button></div>
              <div className="space-y-4">
                 <div><label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Description</label><input type="text" value={newItem.desc} onChange={e => setNewItem({...newItem, desc: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none focus:border-blue-500" /></div>
                 <div className="grid grid-cols-3 gap-4">
                    <div><label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Price</label><input type="number" value={newItem.price} onChange={e => setNewItem({...newItem, price: Number(e.target.value)})} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none" /></div>
                    <div><label className="text-xs font-black text-slate-500 uppercase tracking-widest mb-1 block">Qty</label><input type="number" value={newItem.qty} onChange={e => setNewItem({...newItem, qty: Number(e.target.value)})} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white focus:outline-none" /></div>
                 </div>
                 <button onClick={() => createBillItem.mutate({ visitId: activeInvoice?.visitId || '', patientId: activeInvoice?.patientId || '', description: newItem.desc, quantity: newItem.qty, unitPrice: newItem.price, category: newItem.cat, taxRate: newItem.tax, discountAmount: newItem.discount, isExempt: newItem.isExempt, currency: newItem.currency })} disabled={createBillItem.isPending || !newItem.desc || !newItem.price} className="w-full py-4 mt-4 bg-blue-600 hover:bg-blue-500 rounded-xl font-black text-white transition-all disabled:opacity-50">{createBillItem.isPending ? 'Saving...' : 'Add to Invoice'}</button>
              </div>
           </div>
        </div>
      )}
